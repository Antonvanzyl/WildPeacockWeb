			      ...
			      $.get( "${pageContext.request.contextPath}/getProducts?categoryId=${selectedCategoryId}", function( data ) {
					...
					$("#search").catcomplete({
						...
						change: function( event, ui ) {
							getProductsSearch($( "select.foo" ).val());
						}
					});
			    });
			    
			    function getProductsSearch(searchString)
			    {
			    	 $.get( "${pageContext.request.contextPath}/getProductsSearch?searchString="+searchString, function( data ) {
				    	  var object = jQuery.parseJSON(data);
				    	  $( "#container" ).html( object.product);
					      wall.fitWidth();
				    });
