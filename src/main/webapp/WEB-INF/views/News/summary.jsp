						...
						<tr>
							<td width="600px" style="border-bottom: 1px; border-bottom-style: dashed;">
								<p class="mainText_Contacts">
									<a href="${pageContext.request.contextPath}/news/detail?id=${item.id}" class="mainTextLinkPress"> <b><i>${item.title }</i></b><br /> ${item.subtitle }
									</a>
								</p>
							</td>
							<td style="border-bottom: 1px; border-bottom-style: dashed;"><wp:facebook-like pageURL="${pageContext.request.contextPath}/news/detail?id=${item.id}"></wp:facebook-like>
							</td>
						</tr>
					</c:forEach>
				</table>
